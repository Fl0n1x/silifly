...
class ProgressBarLayout(BoxLayout):
    def __init__(self, progress=50, **kwargs):
        ...
        self.label = Label(text=f"Progress: {progress}%", size_hint=(0.3, 1), color=(0, 1, 0, 1), pos_hint = {'y':.45})
...
class DesighMainScreen(FloatLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.panel = SlidingPanel()  # Створюємо об'єкт SlidingPanel
        self.add_widget(self.panel)  # Додаємо панель на екран
        self.show_layout(None)

    def toggle_panel(self, instance):
        self.panel.toggle_panel(instance)

    def friends(self, instance):
        self.clear_widgets()  # Очищаємо екран
    ...
    def profile(self, instance):
        self.clear_widgets()
        profile_layout = GridLayout(cols=1, spacing=10)

        grid = MyGridLayout()
        self.add_widget(grid)

        avatar = Image(source="default_image.png", size_hint=(1, 0.4))
...
#
 #       history_button = Button(text='History of visiting', size_hint=(1, 0.2))
  #      profile_layout.add_widget(history_button)
        ...
        profile_layout.add_widget(progress_bar)



        self.add_widget(profile_layout)
        optionsButton = Button(background_normal='settingsButton.jpg', size_hint=(None, None), size=(60, 60),
                               pos_hint={'top': 1, 'left': 1})
        optionsButton.bind(on_press=self.options)
    ...
    def options(self, instance):
        ...
        options_layout = BoxLayout(orientation='vertical', padding=10, spacing=10)

        grid = MyGridLayout()
        self.add_widget(grid)

        language_button = Button(text='Languages', font_size=32)
        privacy_button = Button(text='Privacy and Policy', font_size=32)
        promocode_button = Button(text='Input promocode', font_size=32)
        delete_button = Button(text='Delete My Account', font_size=32)


        options_layout.add_widget(language_button)
        options_layout.add_widget(privacy_button)
        options_layout.add_widget(promocode_button)
